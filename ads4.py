...
#creating a UDP socket for ads4
adserver=socket.socket(socket.AF_INET,socket.SOCK_DGRAM)
#binding the socket to the port number startportnum+60 as given in question
adserver.bind((nrip,startportnum+60))
...
while True:
    ...
    #if the domain(message) is in ads4
    if msg in ads:
